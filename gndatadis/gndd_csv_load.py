...
class ParseCSV:
    ...
    def ret_header_dict(self):
        ...
        df_new =self.df
        df_new.columns = filter_chars("\.","_",df_new.columns,replace=True)
        ...
        self.col_dict = {'attr'+str(x+1):df_new.columns[x] for x in range(new_col_len)}
...
class gndwdbCall:
    ...
    def gndwdbMetaNode(self):
        self.meta_ret = gndw_metarepo_metanode_add_api(self.node_name, self.node_attr_list, self.verbose)
        if (self.verbose > 1):
    ...
    def gndwdbDataNodeCall(self):
        ...
        for x in range(df.shape[0]) :
            ...
            self.datanode_name=filter_chars("\.","_",datanode_name,replace=True)
