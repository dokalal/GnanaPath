...
def           gndw_rel_verify_rules(rjson, meta_graph_conn, data_graph_conn, verbose):
    ...
    if (rjson["srcnode"]):
        if (verbose > 2):
            print('gndw_rel_verify_rules: verify if srcnode exists:'+rjson['srcnode'])
        l = gndw_metarepo_metanode_check(meta_graph_conn, rjson['srcnode'], verbose);
        if (l == 0):
            fail = fail + 1;
    else:
        fail = fail+1
    ...
    if (rjson["targetnode"]):
        if (verbose > 2):
            print('gndw_rwl_verify_rules: verify if tgtnode exists:'+rjson['targetnode'])
        l = gndw_metarepo_metanode_check(meta_graph_conn, rjson['targetnode'], verbose);
        if (l == 0):
            fail = fail + 1;
    else:
        fail= fail+1
    ...
    if (rjson["matchnode"]):
        if (verbose > 2):
            print('gndw_rwl_verify_rules: verify if matchnode exists:'+rjson['matchnode'])
        l = gndw_metarepo_metanode_check(meta_graph_conn, rjson['matchnode'], verbose);
        if (l == 0):
            fail = fail + 1;
    else:
        fail= fail+1
...
def            gndw_rel_rules_bizrulesmaster_nrules_get(meta_graph_conn, verbose):
    ...
    with meta_graph_conn.session()  as metaGrpDB_Ses:
        ...
        for n in nodes:
            nr = n["COUNT(r)"];
            print("gndw_rel_rules_bizrulesmaster_nrules_get: number of rules "+str(nr));
        ...
        if (nr == 0):
            ...
            if (len == 0):
                ...
                if (verbose > 3):
                   print("gndw_rel_rules_bizrulesmaster_nrules_get: CREATED BizRulesMaster Node CQLqry:", cqlqry)
...
def          gndw_rel_rules_bizrule_exists(meta_graph_conn, rjson, verbose):
    ...
    with meta_graph_conn.session()   as metaGrpDB_Ses:
             ...
             if (verbose > 2):
                print("gndw_rel_rules_bizrule_exists: BizRule "+rjson["rulename"]+" len:"+str(len))
...
def           gndw_rel_rules_bizrule_relation_exists(meta_graph_conn, rulename,verbose):
        ...
        with meta_graph_conn.session() as metaGrpDB_Ses:
            cqlqry = "MATCH (n)-[r:RULE]-(p {name:'"+str(rulenode)+"'}) return p"
            nodes = metaGrpDB_Ses.run(cqlqry);
            len = 0;
            for  n in nodes:
                len = len + 1;
            if (verbose > 2):
                print('gndw_rel_rules_bizrule_relation_exists: Bix rule for node '+str(rulenode)+' len:'+str(len));
...
def           gndw_rel_rules_bizrule_node_exists(meta_graph_conn, rulename, verbose):
       ...
       with meta_graph_conn.session() as metaGrpDB_Ses:
            ...
            if (verbose > 4):
                print('gndw_rel_rules_bizrule_node_exists: rulenode'+str(rulenode)+' '+str(len)+'  nodes');
...
def           gndw_rel_rules_bizrule_add(meta_graph_conn, rjson, nrules, verbose):
        ...
        with meta_graph_conn.session()   as metaGrpDB_Ses:
             ...
             found = gndw_rel_rules_bizrule_node_exists(meta_graph_conn, rulename, verbose);
        
             if (found == 0):         
                 cqlqry = "MERGE (m:BR_"+str(rulename)+" ";
                 cqlqry += "{ id:'"+str(rid)+"', name:'BR_"+str(rjson["rulename"])+"',";
                 ...
                 if (verbose > 2):
                    print("gndw_rel_rules_bizrule_add: Adding BizRule Node CQLqry:", cqlqry);
                    
             rulename = rjson['rulename'];
            
             rfound =  gndw_rel_rules_bizrule_relation_exists(meta_graph_conn, rulename,verbose);
...
def             gndw_rel_rules_metanode_rule_exists(meta_graph_conn, rjson, verbose):
    ...
    with meta_graph_conn.session() as metaGrpDBSes:
        ...
        cqlqry = "MATCH ()-[r:BRule_"+str(rulename)+" {type:'BizRule'}]-()  return r";
        
        if (verbose > 4):
            print('gndw_rel_rules_metanode_rule_exists: metanode bizrule exists cqlqry:'+cqlqry);
            
        rels = metaGrpDBSes.run(cqlqry);
         
        if (verbose > 2):
            print('gndw_rel_rules_metanodes_add: BR Added '+str(srcnode)+' BR-'+str(rulename)+' '+str(tgtnode))
        len = 0; 
        for r in rels:
           len = len + 1;
...
def             gndw_rel_rules_metanodes_add(meta_graph_conn, rjson, verbose):
    ...
    with meta_graph_conn.session() as metaGrpDBSes:
        ...
        exists = gndw_rel_rules_metanode_rule_exists(meta_graph_conn, rjson, verbose);
        ...
        if (exists == 0):
           ...
           cqlqry = "";        
           cqlqry =  "MATCH (d {name:'"+str(srcnode)+"' , type:'TableNode' }),";
           cqlqry += "(p {name:'"+str(tgtnode)+"'}) "; 
           cqlqry += "MERGE (d)-[:BRule_"+str(rulename)+" {name:'"+str(rulename)+"', type:'BizRule'}]->(p)";
 
           if (verbose > 4):
              print('gndw_rel_rules_metanodes_add: Adding bizrule to meta nodes:'+cqlqry);
...
def              gndw_rel_rules_metarepo_add(rjson, meta_graph_conn, verbose):
    ...
    ##### Metarepo Add Match Rule between source and target node on metarepo with match condition
    with meta_graph_conn.session()    as metaGrpDBSes:
        
        # check if rulenode exists
        nrules = gndw_rel_rules_bizrulesmaster_nrules_get(meta_graph_conn, verbose)
        rid = nrules + 1;
        print('gndw_rel_rules_metarepo_add: Add match condition to rulenode rid:'+str(rid))
...
def            gndw_rel_rules_add(rjson, meta_graph_conn, data_graph_conn, verbose): 
    ...
    retval = gndw_rel_verify_rules(rjson, meta_graph_conn, data_graph_conn, verbose)
