...
def gndw_rel_bizrule_datarepo_datanode_rule_exists(data_graph_conn,
    ...
    with data_graph_conn.session() as dataGrpDB_Ses:
        ...
        if (verbose > 4):
            print(
                'gndw_rel_bizrule_datarepo_datanode_rule_exists: number of rels ' +
                str(nr))
...
def gndw_rel_bizrule_datarepo_datanode_rule_add(data_graph_conn, snodename,
    ...
    with data_graph_conn.session() as dataGrpDB_Ses:
        ...
        if (verbose > 4):
            print(
                'gndw_rel_bizrule_datarepo_datanode_rule_add: New relation' +
                str(relname) +
                '  ')


def gndw_rel_bizrule_datarepo_add(
    ...
    with data_graph_conn.session() as dataGrpDB_Ses:
        ...
        cqlqry = "MATCH (" + str(srcnode) + \
            " {metanode:'" + str(srcnode) + "'}),"
        cqlqry += " (" + str(tgtnode) + " {metanode:'" + str(tgtnode) + "'}), "
        cqlqry += " (" + str(matchnode) + \
            " {metanode:'" + str(matchnode) + "'}) "
        cqlqry += " WHERE " + str(matcond) + " "
        #cqlqry += " return COUNT("+str(srcnode)+"),"+str(srcnode)+", "+str(tgtnode)+" ;";
        cqlqry += "return " + str(srcnode) + ", " + \
            str(tgtnode) + ", " + str(matchnode) + " "
        cqlqry += "  LIMIT 15;"
