...
# Test - Search for  existence and non existence of a Connection based on
# server ip
...
def test_search_op(mocker, load_test_data, test_input, expected):
    ...
    mocker.patch(
        'GnanaPath.gnappsrv.gnp_db_ops.ConnectModel.search_res',
...
def test_insert_op(mocker, load_test_data, test_input, expected):
    _dict, con = load_test_data
    mocker.patch(
        'GnanaPath.gnappsrv.gnp_db_ops.ConnectModel.search_op',
        return_value=test_input)
    assert con.insert_op(_dict) == expected


_newdict = {
    'serverIP': '192.10.10.1:7777',
    'username': 'testneo4j',
    'password': 'pass$123'}
...
# Test update connection record , test the cases with and without server
# ip values
...
# Test filtering the  request header to return only the values related to
# connection params
@pytest.mark.parametrize("test_input, expected",
                         test_list, ids=['Emptydict', 'Extrakey'])
